...
// DBJ: include the Direct3D Library file
...
#include <functional>
/*
NOTE: static functions can not be passed as template value arguments
*/
#define DBJINLINE __forceinline
/*
(c) 2017 by http://github.com/dbjdbj
...
namespace dbj {
	namespace tf {
		...
		typedef void(TigrFunc)(void *) ;

		template<typename TIGRTYPE, TigrFunc cF, TigrFunc rF>
		class Handler {
			TIGRTYPE * tp_ = 0;
		public:
			typedef TIGRTYPE value_type;
			explicit Handler(TIGRTYPE * new_tp) : tp_(new_tp) { cF(tp_); }
			~Handler() { rF(tp_); }
			operator TIGRTYPE * () const noexcept { 
				if (! tp_ ) 
					tigrError(0, "Empty Handler");
				return this->tp_; 
			}
		};
		
		DBJINLINE void creator_empty(void * tp_) {
			if (!tp_)
				tigrError(0, "Can not create pointer to Tigr entity");
		};

		DBJINLINE void releaser_simple(void * tp_) {
			if (tp_) {
				tigrFree(static_cast<Tigr*>(tp_));
				tp_ = 0;
			}
		};
#if 0
		extern const auto creator_empty = std::function<void(void*)>([](void * tp_ ) {
			if (!tp_)
				tigrError(0, "Can not create pointer to Tigr entity");
		});
		extern const auto releaser_simple = std::function<void(void*)>([](void * tp_) {
			if (tp_) {
				tigrFree(static_cast<Tigr*>(tp_));
				tp_ = 0;
			}
...
#endif
		// template<>
		typedef Handler <Tigr,creator_empty, releaser_simple > SimpleHandler;

#if 0		
		//TigrFont *tigrLoadFont(Tigr *bitmap, int codepage);
		extern const auto creator_font = std::function<void(void*)>([](void * font ) {
			if (!font)
				tigrError(0, "Can not load Tigr font");
		});
		extern const auto releaser_font = std::function<void(void*)>([](void * tp_) {
			if (tp_) {
				tigrFreeFont(static_cast<TigrFont*>(tp_));
				tp_ = 0;
			}
		});
#endif
		DBJINLINE void creator_font (void * font) {
			if (!font)
				tigrError(0, "Can not load Tigr font");
		}
		DBJINLINE void releaser_font (void * tp_) {
			if (tp_) {
				tigrFreeFont(static_cast<TigrFont*>(tp_));
				tp_ = 0;
			}
		...
		typedef  Handler <	TigrFont, creator_font, releaser_font	> FontHandler ;
